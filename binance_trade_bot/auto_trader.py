...
class AutoTrader:
    ...
    def transaction_through_bridge(self, pair: Pair, origin_balance_to_use: float = None):
        """
        Jump from the source coin to the destination coin through bridge coin
        """
        can_sell = False
        balance = origin_balance_to_use or self.manager.get_currency_balance(pair.from_coin.symbol)
        from_coin_price = self.manager.get_ticker_price(pair.from_coin + self.config.BRIDGE)
        balance_in_bridge = balance * from_coin_price

        min_notional = self.manager.get_min_notional(pair.from_coin.symbol, self.config.BRIDGE.symbol)
        if balance and balance_in_bridge > min_notional:
            can_sell = True
        else:
        ...
        if (
            can_sell
            and self.manager.sell_alt(pair.from_coin, self.config.BRIDGE, origin_balance_to_use=balance) is None
        ):
