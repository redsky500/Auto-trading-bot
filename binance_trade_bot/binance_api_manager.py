...
class BinanceAPIManager:
    ...
    def retry(self, func, *args, **kwargs):
        ...
        while attempts < 20:
            try:
                ...
                self.logger.warning(f"Failed method: {func}")
                if attempts == 0:
                    pass
    ...
    def _buy_alt(self, origin_coin: Coin, target_coin: Coin):
        ...
        if order_quantity <= 0:
            self.logger.info("Invalid Buy Quantity. Couldn't buy going back to scouting mode.")
            return
    ...
    def sell_alt(self, origin_coin: Coin, target_coin: Coin, origin_balance_to_use: float = None) -> BinanceOrder:
        return self.retry(self._sell_alt, origin_coin, target_coin, origin_balance_to_use=origin_balance_to_use)
    ...
    def _sell_alt(self, origin_coin: Coin, target_coin: Coin, origin_balance_to_use: float = None):
        """
        Sell altcoin
        """
        ...
        if origin_balance_to_use is None:
            origin_balance_to_use = origin_balance

        order_quantity = self._sell_quantity(origin_symbol, target_symbol, origin_balance_to_use)
